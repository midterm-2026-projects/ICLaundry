...
export const createInitialPayment = async ({
  ...
  /**
   * ==============================================
   * DUPLICATE INITIAL PAYMENT CHECK
   * ==============================================
   */

  const existingPayments = await getPaymentsByOrderId(order_id);

  if (Array.isArray(existingPayments) && existingPayments.length > 0) {
    throw new Error("Initial payment already exists.");
  }

  /**
   * Prevent duplicate payment using order status
   */
  ...
  await updateOrderPayment(
    ...
    calculatePaymentStatus(totalPrice, paymentAmount),
...
export const completePayment = async ({ order_id, amount, payment_method }) => {
  ...
  await updateOrderPayment(
    ...
    calculatePaymentStatus(totalPrice, totalAfterPayment),
  ...
  return {
    ...
    paymentStatus: calculatePaymentStatus(totalPrice, totalAfterPayment),
  };
};
